...
def prepare_ml_1m_data(data_path="data/ml-1m/ratings.dat",
    seed=0, second_half_only=True, exclude_train=False):
    ...
    D = create_dataset(event_df, train_df, item_df, horizon, exclude_train=exclude_train)
    D.print_stats()
    V = create_dataset(event_df, valid_df, item_df, horizon, exclude_train=exclude_train)
    return D, V
